<template>
    <div class="md-header">
        <div class="md-title">{{ pageData.page.value.title }}</div>
        <span id="jinrishici-sentence">正在加载今日诗词....</span>
        <div class="md-date">{{formatDate }}</div>
    </div>
</template>
...
export default {
    ...
    computed:{
        formatDate() {
           let time=this.pageData.page.value.lastUpdated
            time = time * 1;     //注意：time应为格式为13位unix时间戳，如果是10位的unix时间戳，需要乘以1000
